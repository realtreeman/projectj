...
$(function(){	
	let getForm = $("#getForm");
	$('#getForm .list').on('click',function(e){ // 목록페이지
		e.preventDefault();
		getForm.empty();
		getForm.append($('#page'));
		getForm.append($('#type'));
		getForm.append($('#keyword'));
		getForm.attr("action", "${contextPath}/board/list/${board.category}");
		getForm.attr("method","get");
		getForm.submit();	
	})
	$('#getForm .modify').on('click',function(e){ // 수정페이지
		e.preventDefault();
		getForm.attr("action","modify");
		getForm.submit();	
	})
	$('#getForm .remove').on('click',function(e){ // 삭제처리
		e.preventDefault();
		getForm.append($('#bno'));
